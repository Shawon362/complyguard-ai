...
export const loader = async ({ params }) => {
  ...
  // Quota status (live calculation)
  const limitInfo = await checkScanLimit(prisma, shop);
  ...
  const monthScanData = await prisma.scan.aggregate({
  ...
  return {
    ...
    rateLimit,
    limitInfo,
    monthImages: monthScanData._sum.imagesProcessed || 0,
  };
};
...
export const action = async ({ params, request }) => {
  ...
  if (intent === "save-quota-override") {
    const scanLimit = formData.get("scanLimit");
    const notes = formData.get("notes") || "";

    if (!scanLimit || parseInt(scanLimit, 10) < 1) {
      return { success: false, message: "Scan limit must be at least 1" };
    }

    await prisma.rateLimitOverride.upsert({
      where: { shop },
      update: {
        monthlyApiLimit: parseInt(scanLimit, 10),
        notes,
        setByAdmin: admin.email,
      },
      create: {
    ...
    await prisma.adminAuditLog.create({
      data: {
        ...
        targetShop: shop,
        details: JSON.stringify({ scanLimit, notes }),
      },
    });

    return { success: true, message: `Custom scan limit set to ${scanLimit}/month` };
  }

  if (intent === "remove-quota-override") {
    await prisma.rateLimitOverride.deleteMany({ where: { shop } });
    await prisma.adminAuditLog.create({
      data: {
        adminEmail: admin.email,
    ...
    return { success: true, message: "Quota override removed. Using plan default." };
  }
...
// ============================================================
export default function MerchantDetail() {
  const { merchant, scans, issues, counts, rateLimit, limitInfo, monthImages } =
    useLoaderData();
  const actionData = useActionData();
  const navigation = useNavigation();
  const isSubmitting = navigation.state === "submitting";
  ...
  const quotaPercent = Math.min(100, (limitInfo.used / limitInfo.limit) * 100);
  const planDefault = limitInfo.isOverride
    ? `(Plan default: ${
        limitInfo.plan === "free"
          ? 1
          : limitInfo.plan === "starter"
            ? 3
            : 15
      })`
    : "";
        ...
        <Layout.Section>
          <div className="admin-stat-grid">
            ...
            <StatCard
              ...
              value={monthImages.toLocaleString()}
            ...
            <Card>
              <BlockStack gap="400">
                <InlineStack align="space-between" blockAlign="center">
                  ...
                  {limitInfo.isOverride && (
                    <Badge tone="info">Custom Limit</Badge>
                  )}
                </InlineStack>
                <Divider />
                ...
                <BlockStack gap="300">
                  ...
                  <ProgressBar
                    ...
                    tone={getQuotaTone(quotaPercent)}
                    size="small"
                  />
                  <InlineStack align="space-between">
                    <Text as="span" variant="bodySm" tone="subdued">
                      {limitInfo.remaining > 0
                        ? `${limitInfo.remaining} scans remaining`
                        : "Limit reached"}
                    </Text>
                    <Text as="span" variant="bodySm" tone="subdued">
                ...
                <BlockStack gap="200">
                  ...
                  <InfoRow
                    label="Max products"
                    value={limitInfo.displayMaxProducts}
                  />
                  <InfoRow
                    label="Max AI images"
                    value={limitInfo.displayMaxImages}
                  />
                  <InfoRow
                    label="Auto-Fix"
                    value={
                      limitInfo.autoFix ? (
                        ...
                        <Badge>{`${limitInfo.freeAutoFixCount} free/scan`}</Badge>
                      )
                    }
                  />
                  <InfoRow
            ...
            <BlockStack gap="400">
              <InlineStack align="space-between" blockAlign="center">
                <BlockStack gap="100">
                  ...
                  <Text as="p" variant="bodySm" tone="subdued">
                    Give this merchant a custom monthly scan limit (overrides plan
                    default)
                  </Text>
                </BlockStack>
              </InlineStack>
              ...
              {limitInfo.isOverride && (
                <Banner tone="info">
                  <Text as="p" variant="bodyMd">
                    Custom limit currently active: <strong>{limitInfo.limit} scans/month</strong>{" "}
                    {planDefault}
                  </Text>
                  {limitInfo.overrideNotes && (
                    <Text as="p" variant="bodySm">
                      Notes: {limitInfo.overrideNotes}
                    </Text>
                  )}
                </Banner>
              )}
              ...
              <Form method="POST">
                <input type="hidden" name="intent" value="save-quota-override" />
                <BlockStack gap="300">
                  <TextField
                    ...
                    helpText="Number of scans this merchant can run per month"
                    autoComplete="off"
                    min={1}
                    requiredIndicator
                  />
                  <TextField
                    ...
                    value={notes}
                    onChange={setNotes}
                    placeholder="e.g. VIP customer, beta tester, internal team"
                    multiline={2}
                    autoComplete="off"
                  />
                  <InlineStack gap="200">
                    <Button submit variant="primary" loading={isSubmitting}>
                      {limitInfo.isOverride ? "Update Override" : "Set Override"}
                    </Button>
                    {limitInfo.isOverride && (
                      <Button
                        submit
